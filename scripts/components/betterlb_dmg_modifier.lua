...
-- 网游常见伤害加成参考:
-- 暴击率: 一般是加算,例如有2个暴击率加成分别为 0.2,0.4 的物品, 最终加成为 0.6
-- 固定伤害加成(基伤): 加算,并且最先计算,没什么好说的
-- 伤害倍率加成: 一般是乘算,并且在加算之后计算,例如有2个伤害倍率加成分别为 1.2,1.4 的物品, 最终加成为 1.68
-- 攻速加成: 一般是加算
...
---@class components
---@field betterlb_dmg_modifier component_betterlb_dmg_modifier

---@class component_betterlb_dmg_modifier
---@field inst ent
...
local betterlb_dmg_modifier = Class(
---@param self component_betterlb_dmg_modifier
---@param inst ent
function(self, inst)
    self.inst = inst
    -- self.val = 0
...
-- function betterlb_dmg_modifier:OnSave()
--     return {
--         -- val = self.val
--     }
-- end

-- function betterlb_dmg_modifier:OnLoad(data)
...
---@param m number
---@param key string
function betterlb_dmg_modifier:ModifierCritical(attri,modifier_type,source,m,key)
...
---@param source ent|string # 来源: 如果是实体,那么实体被移除时, 该修饰也会被移除
---@param key string
function betterlb_dmg_modifier:RemoveModifierCritical(attri,modifier_type,source,key)
    if attri == 'CriticalChance' then
        if modifier_type == 'add' then
...
---@param m number
---@param key string
function betterlb_dmg_modifier:ModifierNormal(dmg_type,modifier_type,source,m,key)
    if modifier_type == 'add' then
...
---@param source ent|string # 来源: 如果是实体,那么实体被移除时, 该修饰也会被移除
---@param key string
function betterlb_dmg_modifier:RemoveModifierNormal(dmg_type,modifier_type,source,key)
    if modifier_type == 'add' then
        if dmg_type == 'physical' then
...
---@return number
---@nodiscard
function betterlb_dmg_modifier:_GetCriticalChanceWithModifier()
...
---@return number
---@nodiscard
function betterlb_dmg_modifier:_GetCriticalDamageWithModifier()
...
function betterlb_dmg_modifier:SetOnCriticalHit(id,fn)
    self.on_critical_hit_fn[id] = fn
end

---移除暴击触发函数
---@param id string
function betterlb_dmg_modifier:RemoveOnCriticalHit(id)
    self.on_critical_hit_fn[id] = nil
end

---运行所有暴击触发函数
---@param victim ent
function betterlb_dmg_modifier:_RunOnCriticalHit(victim)
...
---@param id string
---@param fn fun(victim:ent)
function betterlb_dmg_modifier:SetOnHitAlways(id,fn)
    self.on_hit_fn_always[id] = fn
end

---移除 无论如何都会再击中时触发 的函数
---@param id string
function betterlb_dmg_modifier:RemoveOnHitAlways(id)
    self.on_hit_fn_always[id] = nil
end

---运行所有 无论如何都会再击中时触发 函数
---@param victim ent
function betterlb_dmg_modifier:_RunOnHitAlways(victim)
    for _,v in pairs(self.on_hit_fn_always) do
        v(victim)
    end
end

return betterlb_dmg_modifier