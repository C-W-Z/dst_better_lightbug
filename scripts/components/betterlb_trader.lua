...
---@class components
---@field betterlb_trader component_betterlb_trader # 这个组件是为 通用交易 功能服务的 <br> 禁止自己添加这个组件, 方法可以调用

local function on_val(self, value)
    self.inst.replica.betterlb_trader.val:set(value)
end

---@class component_betterlb_trader
---@field inst ent
---@field val string
---@field _last_decode_res integer[]|nil
local betterlb_trader = Class(
---@param self component_betterlb_trader
---@param inst ent
function(self, inst)
    self.inst = inst
    self.val = ''
...
function betterlb_trader:OnSave()
    local data = {}
    data.val = self.val
    return data
end

function betterlb_trader:OnLoad(data)
...
---@param doer ent
---@return boolean
function betterlb_trader:AddOne(item,doer)
...
---@return integer
---@nodiscard
function betterlb_trader:GetCurrentNum(prefab)
...
---@return integer[]
---@nodiscard
function betterlb_trader:decode(encoded_str, separator)
    local parts = {}
...
---@return string
---@nodiscard
function betterlb_trader:encode(separator, args)
    return table.concat(args, separator or '|')
end

return betterlb_trader