...
---@class components
---@field betterlb_cd_in_itemtile component_betterlb_cd_in_itemtile

---comment
---@param self replica_betterlb_cd_in_itemtile
---@param value any
local function on_cur_cd(self, value)
    self.inst.replica.betterlb_cd_in_itemtile:SetCurCD(value)
end
...
local function on__show_itemtile(self, value)
    self.inst.replica.betterlb_cd_in_itemtile._show_itemtile:set(value)
end

---@class component_betterlb_cd_in_itemtile
---@field inst ent
...
local betterlb_cd_in_itemtile = Class(
---@param self component_betterlb_cd_in_itemtile
---@param inst ent
function(self, inst)
    self.inst = inst
    self.cd = 10
...
function betterlb_cd_in_itemtile:OnSave()
    return {
        cd = self.cd,
        cur_cd = self.cur_cd,
    }
...
function betterlb_cd_in_itemtile:OnLoad(data)
...
function betterlb_cd_in_itemtile:Init(cd,shown_on_itemtile)
    self.cd = cd
...
---更改默认cd
---@param cd integer
function betterlb_cd_in_itemtile:ChangeDefaultCD(cd)
    self.cd = cd
end
...
---@protected
function betterlb_cd_in_itemtile:_StartCD(cd,start_without_onstartcdfn)
    -- 更新默认cd
    if cd then
        self.cd = cd
    end
...
---开始cd
---@param cd integer|nil # 不填用默认cd,填了则会更新默认cd
function betterlb_cd_in_itemtile:StartCD(cd)
...
---@return boolean
---@nodiscard
function betterlb_cd_in_itemtile:IsCD()
    return self.cur_cd > 0
end

---当在cd中时修改剩余cd
---@param val integer
function betterlb_cd_in_itemtile:SetCurCD(val)
    if self:IsCD() then
        self.cur_cd = val
    end
end
...
function betterlb_cd_in_itemtile:SetOnStartCD(fn)
    self.onstartcdfn = fn
end

---设置cd转好时调用的函数
---@param fn fun(this:ent,...):...
function betterlb_cd_in_itemtile:SetOnFinishCD(fn)
    self.onfinishcdfn = fn
end

---是否显示在itemtile上,不设置默认显示
---@param shown boolean
function betterlb_cd_in_itemtile:ShowItemTile(shown)
    self._show_itemtile = shown
end

return betterlb_cd_in_itemtile