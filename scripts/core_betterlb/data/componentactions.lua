...
local trade_data = require('core_betterlb/data/trade')

---@type data_componentaction[]
local data = {
    -- 官方的三个组件的修理, finiteuses armor fueled, 给一堆物品时, 尽可能多的消耗物品来修理, 具体数据只需填进 `TUNING.MOD_BETTERLB.REPAIR_COMMON`, 此处逻辑不用动
    {
        id = 'ACTION_BETTERLB_COMMON_REPAIR',
        str = STRINGS.MOD_BETTERLB.ACTIONS.ACTION_BETTERLB_COMMON_REPAIR,
        fn = function (act)
            ...
            if doer and item and target and doer:IsValid() and item:IsValid() and target:IsValid() then
                local compo = TUNING.MOD_BETTERLB.REPAIR_COMMON[target.prefab].type
                local repair_percent = item.prefab and TUNING.MOD_BETTERLB.REPAIR_COMMON[target.prefab].repair_percent[item.prefab]
                ...
                if repair_percent and cur_percent then
                    ...
                    SUGAR_betterlb:consumeOneItem(item)
        ...
        testfn_type_USEITEM = function (inst, doer, target, actions, right)
            if right and doer:HasTag("player") and target.prefab and TUNING.MOD_BETTERLB.REPAIR_COMMON[target.prefab] then
                local canrepair = inst and inst.prefab and TUNING.MOD_BETTERLB.REPAIR_COMMON[target.prefab].repair_percent[inst.prefab]
                if canrepair then
                    return true
                end
            end
    ...
    },
    {
        ...
        str = function (act)
            ...
            local _str = _tbl and _tbl.str or STRINGS.MOD_BETTERLB.ACTIONS.ACTION_BETTERLB_TRADE
            return _str
        end,
        fn = function (act)
            ...
            if target and item and doer and target:IsValid() and item:IsValid() and doer:IsValid() then
                ...
                if target.components.betterlb_trader then
                    return target.components.betterlb_trader:AddOne(item,doer)
                end
            end
            return false
        end,
        ...
        testfn_type_USEITEM = function (inst, doer, target, actions, right)
            ...
            if fixed and target.replica.betterlb_trader then
                ...
                return not target.replica.betterlb_trader:IsMax(i,max) and res_testfn
            end
            return false
        end
    },
