...
-- 此处无需做任何更改, 请到 `core_betterlb/data/trade.lua` 中进行填表
---@source ../../core_betterlb/data/trade.lua:40 
local _______see -- ctrl + 左键 点我快速跳转

local tbl = require('core_betterlb/data/trade')

AddReplicableComponent('betterlb_trader')
...
for tar,v in pairs(tbl.map) do
    ...
    for itm,v2 in pairs(v) do
        ...
        AddPrefabPostInit(itm,function (inst)
            ...
            if inst.components.betterlb_tradeable == nil then
                inst:AddComponent('betterlb_tradeable')
            end
        end)
    end
    AddPrefabPostInit(tar,function (inst)
        ...
        if inst.components.betterlb_trader == nil then
            inst:AddComponent('betterlb_trader')
            local need_encode = {}
            for _i = 1,itm_num do
                table.insert(need_encode,0)
            end
            inst.components.betterlb_trader.val = inst.components.betterlb_trader:encode(nil,need_encode)
        end
    end)
end
...
function (self)
    ...
    function self:DoActionAutoEquip(buffaction,...)
        if buffaction.action == ACTIONS.ACTION_BETTERLB_TRADE then
            return false
        end
